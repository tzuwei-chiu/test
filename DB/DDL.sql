...
CREATE TABLE `comments` (
  ...
  KEY `comments_fk_1` (`userId`),
  KEY `comments_fk_2` (`postId`),
  CONSTRAINT `comments_fk_1` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`),
  CONSTRAINT `comments_fk_2` FOREIGN KEY (`postId`) REFERENCES `posts` (`postId`) ON DELETE CASCADE
...
CREATE TABLE `posts` (
  ...
  `createdAt` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`postId`),
  KEY `posts_fk_1` (`userId`),
  CONSTRAINT `posts_fk_1` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`) ON DELETE CASCADE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addComment`(
    IN in_userId INT,
    IN in_postId VARCHAR(255),
    IN in_content TEXT,
    OUT out_commentId INT
)
BEGIN
    ...
    SET out_commentId = LAST_INSERT_ID();
END ;;
DELIMITER ;
...
BEGIN
    DELETE FROM posts WHERE postId = in_postId;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `getOthersPosts` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getOthersPosts`(
    IN in_userId INT
)
BEGIN
    SELECT p.*, u.userName
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getPost`(
    IN in_postId INT
)
BEGIN
    SELECT p.*, u.userName
...
BEGIN
    SELECT * FROM users WHERE userId = in_userId;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getUserId`(
    IN in_phoneNum VARCHAR(20),
    OUT out_userId VARCHAR(255)
)
BEGIN
    SELECT userId INTO out_userId
    FROM users
    WHERE phoneNum = in_phoneNum;
...
BEGIN
    SELECT userName FROM users WHERE userId = in_userId;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `register`(
    ...
    IN in_phoneNum varchar(20),
    IN in_pass varchar(255),
    IN in_coverImage VARCHAR(255),
    IN in_biography TEXT
)
BEGIN
    INSERT INTO users (
        userName,
...
BEGIN
    UPDATE posts
    SET content = in_content, image = in_image
...
-- Dump completed on 2025-05-14 21:54:51
